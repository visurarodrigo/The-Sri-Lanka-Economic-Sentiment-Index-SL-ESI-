...
# Correctly define the project root
...
def fetch_macro_data():
    """Fetch USD/LKR exchange rate and Inflation from FRED API using direct CSV download."""
    ...
    try:
        ...
        exchange_df = pd.read_csv(url_exch, parse_dates=['observation_date'], index_col='observation_date')
        exchange_df = exchange_df['2018-01-01':].copy()
        
        # Resample to monthly mean to align with other monthly data
        exchange_df = exchange_df.resample('ME').mean().reset_index()
        exchange_df['year_month'] = exchange_df['observation_date'].dt.to_period('M').astype(str)
        exchange_df = exchange_df[['year_month', 'DEXSLUS']].rename(columns={'DEXSLUS': 'usd_lkr_rate'})
        ...
        inflation_df = pd.read_csv(url_inf, parse_dates=['observation_date'], index_col='observation_date')
        inflation_df = inflation_df['2018-01-01':].copy()
        
        # Resample to monthly mean (in case of any irregularities)
        inflation_df = inflation_df.resample('ME').mean().reset_index()
        inflation_df['year_month'] = inflation_df['observation_date'].dt.to_period('M').astype(str)
        inflation_df = inflation_df[['year_month', 'FPCPITOTLZGLKA']].rename(columns={'FPCPITOTLZGLKA': 'inflation_rate'})
        ...
        print("  Creating placeholder data. Please check your internet connection.")
